...
try {
    ...
    $authoringDirectory = New-Item -ItemType Directory -Force -Path (Join-Path $packageRoot "Mods/S1Lua")
    $starterDirectory = New-Item -ItemType Directory -Force -Path (Join-Path $authoringDirectory "_StarterMod")
    $vscodeDirectory = New-Item -ItemType Directory -Force -Path (Join-Path $authoringDirectory ".vscode")
    $editorDirectory = New-Item -ItemType Directory -Force -Path (Join-Path $authoringDirectory "Editor")
    ...
    Copy-Item -LiteralPath "examples/MyFirstMod/mod.lua" -Destination (Join-Path $starterDirectory "mod.lua")
    Copy-Item -LiteralPath "packaging/authoring/.luarc.json" -Destination (Join-Path $authoringDirectory ".luarc.json")
    Copy-Item -LiteralPath "packaging/authoring/.vscode/extensions.json" -Destination (Join-Path $vscodeDirectory "extensions.json")
